...
describe('getWeekDates', () => {
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연말)', () => {
    ...
    expect(getWeekDates(endYear)).toEqual([
      new Date('2024-12-30'),
      new Date('2024-12-31'),
      new Date('2025-01-01'),
      new Date('2025-01-02'),
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연초)', () => {
    const beginningYear = new Date('2025-01-01');

    expect(getWeekDates(beginningYear)).toEqual([
      new Date('2024-12-30'),
      new Date('2024-12-31'),
      new Date('2025-01-01'),
      new Date('2025-01-02'),
  ...
  it('윤년의 2월 29일을 포함한 주를 올바르게 처리한다', () => {
    const leapYearThursday = new Date('2024-02-29');

    expect(getWeekDates(leapYearThursday)).toEqual([
      new Date('2024-02-26'),
      new Date('2024-02-27'),
      new Date('2024-02-28'),
      new Date('2024-02-29'),
  ...
  it('월의 마지막 날짜를 포함한 주를 올바르게 처리한다', () => {
    ...
    expect(getWeekDates(lastDate)).toEqual([
      new Date('2024-01-29'),
      new Date('2024-01-30'),
      new Date('2024-01-31'),
      new Date('2024-02-01'),
...
describe('formatWeek', () => {
  it('월의 중간 날짜에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-11-15'))).toBe('2024년 11월 3주');
  });

  it('월의 첫 주에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-11-01'))).toBe('2024년 11월 1주');
  });
...
describe('formatMonth', () => {
  it('2024년 7월 10일을 "2024년 7월"로 반환한다', () => {
    expect(formatMonth(new Date('2024년 7월 10일'))).toBe('2024년 7월');
  });
});
...
describe('isDateInRange', () => {
  ...
  it('범위 내의 날짜 2024-07-10에 대해 true를 반환한다', () => {
    expect(isDateInRange(new Date('2024-7-10'), rangeStart, rangeEnd)).toBe(true);
  });

  it('범위의 시작일 2024-07-01에 대해 true를 반환한다', () => {
    expect(isDateInRange(new Date('2024-7-01'), rangeStart, rangeEnd)).toBe(true);
  });

  it('범위의 종료일 2024-07-31에 대해 true를 반환한다', () => {
    expect(isDateInRange(new Date('2024-7-31'), rangeStart, rangeEnd)).toBe(true);
  });
...
describe('formatDate', () => {
  it('날짜를 YYYY-MM-DD 형식으로 포맷팅한다', () => {
    expect(formatDate(new Date('2024-11-06'))).toBe('2024-11-06');
  });

  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    expect(formatDate(new Date('2024-11-06'), 10)).toBe('2024-11-06');
  });
